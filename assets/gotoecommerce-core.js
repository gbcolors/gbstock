  ...
  function mount(target, config) {
    ...
    function renderShell() {
      root.innerHTML = [
        ...
        '<a class="ge-brand" href="' + safe(config.basePath || "./") + '">' + (config.logoUrl ? '<img class="ge-logo-img" src="' + safe(config.logoUrl) + '" alt="' + safe(config.logoText || "GB") + '">' : '<span class="ge-logo">' + safe(config.logoText || "GB") + '</span>') + '<span><strong class="ge-wordmark" style="font-family:' + safe(config.wordmarkFontFamily || config.displayFontFamily || "GBstock Plus Display") + ', Inter, sans-serif;">' + safe(config.brandName) + '</strong>' + (config.brandMeta ? '<small>' + safe(config.brandMeta) + '</small>' : '') + '</span></a>',
        ...
        '</header>',
        '<section class="ge-hero"><div class="ge-hero-copy"><p class="ge-eyebrow">' + safe(config.eyebrow) + '</p><div class="ge-hero-type" style="font-family:' + safe(config.wordmarkFontFamily || config.displayFontFamily || "GBstock Display") + ', Inter, sans-serif;">stock</div><h1>' + safe(config.headline) + '</h1><p class="ge-muted">' + safe(config.subhead) + '</p><div class="ge-search"><div><strong>Que estas buscando</strong><input id="' + tenant + '-search" type="search" placeholder="' + safe(config.searchPlaceholder) + '"></div><div><strong>Para que lo necesitas</strong><select id="' + tenant + '-intent"><option value="">Selecciona un uso</option><option value="hotel">Hotel / hospitality</option><option value="restaurant">Restaurante / cafe</option><option value="anuncio">Anuncio / campana</option><option value="logo">Logo / identidad</option><option value="reels">Reels / contenido</option><option value="app">App / interfaz</option></select></div><button data-ge-search>Buscar</button></div><div class="ge-search-chips"><button class="ge-chip" data-ge-chip="sans">Sans serif</button><button class="ge-chip" data-ge-chip="italic">Italicas</button><button class="ge-chip" data-ge-chip="display">Display</button><button class="ge-chip" data-ge-chip="licencia">Licencia comercial</button><button class="ge-chip" data-ge-chip="musica anuncio">Musica para anuncio</button><button class="ge-chip" data-ge-chip="efectos">Efectos</button></div><div class="ge-stats"><span><strong>1</strong> familia curada</span><span><strong>6</strong> estilos</span><span><strong>3</strong> licencias</span></div></div>',
        ...
        '<section class="ge-editorial"><div><p class="ge-eyebrow">Curated for commerce</p><h2>Menos volumen, mas criterio: solo fuentes que puedan defenderse como producto real.</h2></div><p>El catalogo tipografico queda en curaduria activa. Adebayo Sans abre la linea con paquete familiar, licencias y descarga digital; las familias generadas de relleno salen de venta hasta redisenarse con calidad comercial.</p></section>',
        '<section class="ge-specimen"><div class="ge-section-head"><div><p class="ge-eyebrow">Adebayo / stock display</p><h2>Familia completa para revision</h2></div><span class="ge-count">OTF / TTF / WOFF2</span></div><div class="ge-specimen-word" style="font-family:' + safe(config.wordmarkFontFamily || config.displayFontFamily || "GBstock Plus Display") + ', Inter, sans-serif;">stock</div><div class="ge-specimen-lines" style="font-family:' + safe(config.displayFontFamily || "GBstock Plus Display") + ', Inter, sans-serif;"><p>abcdefghijklmnopqrstuvwxyz</p><p>ABCDEFGHIJKLMNOPQRSTUVWXYZ</p><p>0123456789 . - / &</p><p>Sans Serif Italicas Cursivas Display Gratis</p></div></section>',
    ...
    function renderFontRow(product) {
      ...
      var fontFamily = config.displayFontFamily || "GBstock Plus Display";
      var fontStyle = ' style="font-family:' + safe(fontFamily) + ', Inter, sans-serif;"';
